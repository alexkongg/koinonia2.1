...
*/
(function(d){Galleria.addTheme({name:"folio",author:"Galleria",version:"1.0",css:"galleria.folio.css",defaults:{transition:"pulse",thumbCrop:"width",imageCrop:false,carousel:false,show:false,easing:"galleriaOut",thumbEventType:Galleria.TOUCH?"mousedown":"click",_webkitCursor:true,_animate:true},init:function(e){this.addElement("preloader","loaded","close").append({container:"preloader",preloader:"loaded",stage:"close"});var i=this,u=this.$("stage"),j=this.$("thumbnails"),r=this.$("images"),m=this.$("info"),
...
var b=a.thumbTarget,c=this.$("plus").css({display:"block"}).insertAfter(b),k=0;e.showInfo&&this.hasInfo(a.index)&&c.append("<span>"+this.getData(a.index).title+"</span>");p=p||d(b).parent().outerWidth(true);d(b).css("opacity",0);Galleria.IE?c.hide():c.css("opacity",0);
/*d(b).parent().hover(function(){Galleria.IE?c.show():c.stop().css("opacity",1)},function(){Galleria.IE?c.hide():c.stop().animate({opacity:0},300)}).data("index",a.index);*/
s++;this.$("loaded").css("width",s/this.getDataLength()*100+"%");
...
{top:-50})}this.$("container").css({width:e.width,height:"auto"});e._webkitCursor&&Galleria.WEBKIT&&this.$("image-nav-right,image-nav-left").addClass("cur");this.$("close").click(function(){i.exitFullscreen()});
/*d(window).resize(
    function(){
        if(t)i.getActiveImage()&&y(i.getActiveImage().width);
            ...
            if(a!==w){
                w=a;
                A(j,{width:p,delay:50,debug:true,onheight:function(){o.height(j.height())}})
            }
        }
    })*/
    }})})(jQuery);